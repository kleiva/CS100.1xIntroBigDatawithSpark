...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    avg = float(sum(IDandRatingsTuple[1]))/num
...
"""movieWithAvgRatingsRDD = (moviesRDD
                          .join(movieIDsWithAvgRatingsRDD)
                          .filter(lambda (ID, (name, (num, avg))) : num > 500)
                          .map(lambda (ID, (name, (num, avg))): (avg, (ID, name,num)) )
                          .sortByKey())
                          
print movieWithAvgRatingsRDD.takeOrdered(50, lambda (k,v): -1*k)"""
